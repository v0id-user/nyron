...
2. **Refreshes `.nyron/` state from `package.json`** so the working package version stays the source of truth
3. **Increments the version** in `.nyron/meta.json` for the selected project
4. **Records the version transition** in `.nyron/versions.json`
5. **Updates `package.json`** in the project directory to match the new version
