...
This creates a `nyron.config.ts` file plus `.nyron/meta.json`, `.nyron/versions.json`, and matching local JSON Schema files inside `.nyron/`.
...
You only need GitHub authentication when you want to publish a GitHub release:
...
<Callout type="info">
You can still run `nyron init`, `nyron bump`, `nyron push-tag`, and `nyron release --dry-run` without `GITHUB_TOKEN`. GitHub authentication is required when you publish the release to GitHub, and Nyron can also fall back to `gh auth login`.
</Callout>
...
The `--use-existing-tag` flag tells Nyron to use the existing nyron-release tag you just pushed, rather than creating a new one. On a first release, Nyron will use repository history from the start of the repo up to that tag.
...
# 4. Commit the updated state files
git add .nyron/meta.json .nyron/versions.json
git commit -m "chore(nyron): update release tag"

# 5. Create GitHub release (use --use-existing-tag since tag already exists)
npx @nyron/cli release --use-existing-tag
```
