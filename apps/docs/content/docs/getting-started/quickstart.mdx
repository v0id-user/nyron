...
**Why use `-n`?** The `-n` (or `--no-tag`) flag tells Nyron to use the nyron-release tag you just pushed with `push-tag`, rather than creating a duplicate tag. When using `-n`, the command fetches commits BETWEEN the previous tag and the tag you just pushed. This is especially important when:
...
**Important**: When using `release` without `-n`, it creates a new nyron-release tag AFTER the release is published. This tag marks the boundary for the NEXT release. Make sure to push tags with `git push --follow-tags` or `git push && git push --tags`.
