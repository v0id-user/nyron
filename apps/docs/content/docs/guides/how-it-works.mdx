...
**Mode 1: Existing Tag Release (with `--use-existing-tag`)**
...
**Mode 2: Standard Release (without `--use-existing-tag`)**
...
**At least one nyron-release tag is required** - Create your first tag using `push-tag`, then use `release --use-existing-tag`. If there is only one Nyron release boundary tag, Nyron uses repository history from the start of the repo up to that tag.
</Callout>
...
- **With `--use-existing-tag`**: Fetches commits **between** the previous tag and the latest tag
- **Without `--use-existing-tag`**: Fetches commits from the latest tag to `HEAD`
...
- Enriches with GitHub data (author usernames, avatars, commit URLs) if GitHub authentication is configured
...
- **Wet Run**: Creates the actual GitHub release with the generated changelog as body
- Uses the determined release tag (pushed tag with `--use-existing-tag`, latest tag without it)
- Publishes to the repository's releases page

### Step 7: Tag Creation (Next Release Boundary)

**Only executed without `--use-existing-tag` and not in dry-run mode:**
...
- Created by `nyron push-tag` or `nyron release` (without `--use-existing-tag`)
...
- **`nyron release` (without `--use-existing-tag`)**: Creates a new tag after the release is published. This tag marks the boundary for the next release, not the current one.
- **`nyron release --use-existing-tag`**: Does not create a tag. Uses the existing tag you pushed with `push-tag` and fetches commits between the previous tag and the tag you just pushed. On a first release, it falls back to repository start.
...
<Callout type="warn">
**For existing repositories**: If you're starting with Nyron on a repo that already has many commits and tags, create an initial `nyron-release@*` tag at your desired starting point using `push-tag`. Then publish the first release with `release --use-existing-tag`. Future releases will include commits after that boundary.
</Callout>
...
GitHub is required when you publish a GitHub release. Nyron can use `GITHUB_TOKEN`, `GH_TOKEN`, or a GitHub CLI session from `gh auth login`. Local setup, version bumps, tag pushes, and `--dry-run` release previews can still work from local git history without GitHub authentication.
