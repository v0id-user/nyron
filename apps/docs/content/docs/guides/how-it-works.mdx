...
**Tag Creation Timing**:
- **`nyron push-tag`**: Creates and pushes the tag immediately
...
- **`nyron release` (with `-n`)**: Does NOT create a tag. Uses the existing tag you pushed with `push-tag`.

When you run `nyron release`:
1. It finds the latest `nyron-release@*` tag (and previous tag if using `-n`)
2. Fetches commits since that tag to HEAD (without `-n`) or between tags (with `-n`)
3. Parses those commits to generate the changelog
4. Creates a GitHub release
5. **Creates a new nyron-release tag** (only without `-n` flag and not in dry-run) - This tag marks where the NEXT release will start from
