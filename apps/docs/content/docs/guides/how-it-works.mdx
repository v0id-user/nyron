...
- **`nyron release --use-existing-tag`**: Does not create a tag. Uses the existing tag you pushed with `push-tag` and fetches commits between the previous tag and the tag you just pushed.
...
GitHub is required when you publish a GitHub release. Local setup, version bumps, tag pushes, and `--dry-run` release previews can still work from local git history without GitHub authentication.
