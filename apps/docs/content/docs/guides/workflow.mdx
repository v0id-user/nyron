...
<Callout type="info">
**Understanding Release Boundaries**: Nyron uses `nyron-release@*` tags to determine which commits to include in releases. These tags are separate from your project version tags (like `v1.0.0`). When you run `release`, Nyron looks for the latest `nyron-release@*` tag and includes all commits since that tag (or between tags if using `--use-existing-tag`). Your existing tags won't interfere with this process.
</Callout>
...
# Note: This requires an existing nyron-release boundary.
# For a first release, run `nyron push-tag` first and then use `nyron release --use-existing-tag`.
...
<Callout type="warn">
**First-time use**: For your first release, create a `nyron-release@*` boundary with `push-tag`, then publish with `nyron release --use-existing-tag`. Nyron will include commits from the beginning of repository history up to that first boundary tag.
</Callout>
...
**Understanding the `--use-existing-tag` flag**: When you use `push-tag`, it creates a `nyron-release@*` tag. The `--use-existing-tag` flag tells `release` to:
...
- On a first release, fall back to repository start if no previous boundary exists
- NOT create a new tag after the release

Without `--use-existing-tag`, `release` would fetch commits from the latest tag to `HEAD` and create the next boundary tag after publishing, which isn't what you want when you've already pushed a tag.
...
**Note:** The `--use-existing-tag` flag tells the release command to use the existing `nyron-release@*` tag you pushed with `push-tag`, rather than creating a duplicate tag. This is essential when using CI/CD workflows.
...
git push  # CI/CD creates the release for you (using --use-existing-tag)
```
...
- ✅ No need to have `GITHUB_TOKEN` locally if you publish from CI
